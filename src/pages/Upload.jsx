...
function Upload() {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:9000/api/upload/getall?page=${currentPage}`
      );
      setList(response.data.data);
      setTotalPages(response.data.totalPages);
      setCurrentPage(response.data.currentPage);
  ...
  return (
    <div className="p-4 sm:ml-64">
      <div className="p-4 border-dashed rounded-lg mt-5">
        {/* File Upload Section */}
        <div className="flex flex-col items-center max-w-screen-md mx-auto mt-10 mb-3 p-4 border-2 border-gray-200 bg-white rounded-lg">
